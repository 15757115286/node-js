(function(global, factory) {
  ...
  function initScrollBar(containerId, contentId, options) {
    ...
    box.addEventListener('mouseenter',function(e){
      var container = box.querySelector('.' + containerClass);
      if(container){
        container.classList.add(prefix + '-hover');
      }
    });
    box.addEventListener('mouseleave',function(e){
      var container = box.querySelector('.' + containerClass);
      if(container){
        container.classList.remove(prefix + '-hover');
      }
    });
  ...
  function initVerticalScrollBar(box, content, options) {
    ...
    box.addEventListener(
      "mousewheel",
      (options.mousewheelFn = function(e) {
    ...
    if (oldScrollBar) {
      var moveY = Math.min(
        parseInt(oldScrollBar.style.top),
        boxHeight - barHeight
      );
      toMove(moveY);
    }
  ...
  // 初始化水平滚动条
  function initHorizontalScrollBar(box, content, boxWidth) {
